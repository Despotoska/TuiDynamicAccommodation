...
public class AddNewAirport 
{
	...
	@Test
	public void AddNewAirportTest() throws InterruptedException
	{
...
//		Expand the country
		...
		boolean airportExistsAlready = driver.findElements(By.xpath("//a[contains(text(),'"+TestData.AIRPORTNAME1+"')]")).size() > 0;
		
		//If the airport is new, then add "New Airport" from the drop down menu
		if(!airportExistsAlready)
		{
			...
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("ul.uip-menu li:nth-of-type(6)"))).click();
//	     	Fills the form with airport name, IATA code
			driver.switchTo().parentFrame();
			driver.switchTo().frame("tvcInlineFrame_0");
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("input#f17"))).sendKeys(TestData.AIRPORTNAME1);
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("input#f18"))).sendKeys(TestData.IATACODE1);
		...
		//Test that the airport now exists (regardless if it was added by the script or pre-existed)
		driver.switchTo().defaultContent();
		driver.switchTo().frame("content");
		driver.switchTo().frame("tvcTabs0contentFrame");
		driver.switchTo().frame("tableContentFrame");			   
		driver.switchTo().frame("tableBodyRight");
		assertTrue(driver.findElements(By.xpath("//a[contains(text(),'"+TestData.AIRPORTNAME1+"')]")).size() > 0);
